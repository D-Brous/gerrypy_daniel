...
class Logger:
    def __init__(self, logging, printing, debug_file_path):
    ...
    @if_logging
    def log_internal_node(self, id):
        return f"Logged internal node {id} at time {time.thread_time()}\n"

    @if_logging
    def log_leaf_node(self, id):
        return f"Logged leaf node {id} at time {time.thread_time()}\n"

    def log_node_deletion(self, id):
        if self.logging:
            self.buffer.append(id)

    @if_logging
    def log_failed_partition(self, id):
        buffer = self.buffer
        self.bugger = []
        return f"Failed split of node {id}.\n    Deleted nodes:\n    {buffer}"

    @if_logging
    def log_failed_root_partition(self, str):
        return f"\n-------------------------------Root partition failed ({str})-------------------------------"

    @if_printing
    def print_failed_root_partition(self, str):
        return f"\n-------------------------------Root partition failed ({str})-------------------------------"
    ...
    @if_logging
    def log_n_nodes(self, n_nodes):
        return (
            f"\n    Total number of nodes in tree after deletion: {n_nodes}\n"
        )

    @if_logging
    def log_partition(self, id, children, n_trials, t_init):
    ...
    def print_completed_root_partition(
        ...
        return (
            f"\nGeneration time: {tree.generation_times[root_partition_id]}\n"
    ...
    @if_printing
    def print_beta_reopt_time(self, time: float):
        return f"Total beta reoptimization time: {time}"
    ...
    @if_printing
    def print_generation_completion(self, config: SHPConfig, tree: SHPTree, partitions: Partitions):
        string = (
            "\n<><><><><><><><><><><> SHP Algorithm End <><><><><><><><><><><>\n"
        + f"Number of internal nodes: {tree.n_internal_nodes()}\n"
        + f"Number of leaf nodes: {tree.n_leaf_nodes()}\n"
        ...
        for partition_id in range(n_partitions):
            ...
            n_maj_cvap_arr[partition_id] = n_maj_cvap(config.col, partition, demo_df)
